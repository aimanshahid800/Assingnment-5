...
function claculateArea(r: number) {
  const PI = 3.14;
  let area = PI * r ** 2;
  return area;
}
...
let temprature2: number[] = [20, 30, 49, 79, 21];
...
function arrayOfNnumbers(arr: number[]) {
...
console.log(
  "the positive numbers are in array is:",
  arrayOfNnumbers([2, -3, 4, -5, 6])
);

//Question-11

console.log("Question-11");
function wordStartWithA(word: string[]) {
...
let fruit: string[] = ["apple", "banana", "mango", "orange", "strawberry"];
let secondToLastFruit: string = fruit[fruit.length - 2];
console.log(secondToLastFruit);

//Question-13

console.log("Question-13");
function squreOfArray(num: number[]) {
...
function countScoreChanges(scores: number[]) {
...
function sumOfElements(arr: number[], parity: Parity): number {
...
console.log(
  "Product of all elements of array is",
  calculateProduct([1, 4, 6, 8, 4])
);
...
function minMaxAverage(nums: number[]): {
  min: number;
  max: number;
  avg: number;
} {
...
function logReverse(arr: any[]): void {
  for (let i = arr.length - 1; i >= 0; i--) {
    console.log(arr[i]);
  }
}
